...
CREATE TABLE `task` (
  ...
  `use_num` int(11) DEFAULT NULL COMMENT '使用次数',
  `watch_num` int(11) DEFAULT NULL COMMENT '查看次数',
  PRIMARY KEY (`task_id`),
  KEY `task_category` (`task_category`),
  CONSTRAINT `task_category` FOREIGN KEY (`task_category`) REFERENCES `point` (`task_category`) ON UPDATE CASCADE
