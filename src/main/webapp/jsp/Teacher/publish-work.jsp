...
<script type="text/javascript">
	...
	$(document)
			.ready(
					function() {
						...
						$("#selectTypeSelect li")
								.click(
										function() {
											...
											$tasktype = $("#taskCategorySelect")
													.val();
											$('#taskListTable')
													.bootstrapTable(
															'refresh',
															{
																url : "${pageContext.request.contextPath}/teacher/getTaskListPage?taskCategory="
																		+ $tasktype
															});
										});
						...
						$("#editTime .wui-date-editor input").attr("name",
								"taskEndTime");
						$("#editTime .wui-date-editor input").attr("id",
								"taskEndTimeEdit");
						$("#selectTime .wui-date-editor input").attr("name",
								"taskEndTime");
						$("#selectTime .wui-date-editor input").attr("id",
								"taskEndTimeSelect");
															...
															'click #preview' : function(
																...
																$
																		.ajax({
																			async : false,
																			cache : false,
																			url : "${pageContext.request.contextPath}/teacher/ajaxGetTaskPerview?taskId="
																					+ row.taskId,
																			type : "POST",
																			dataType : "json",
																			success : function(
																				...
																				for (var i = 0; i < accessoryList.length; i++) {
																					$(
																							"#autoAccessoryList")
																							.append(
																									"<a href='${pageContext.request.contextPath}/teacher/resourceDownload?fileName="
																											+ accessoryList[i].accessoryName
																											+ "&id="
																											+ result[0].taskId
																											+ "&type=teacherPub'>"
																											+ "<button class='btn btn-default' type='submit' style='border-radius: 20px;float: left;margin-left: 10px;' >"
																											+ accessoryList[i].accessoryName
																											+ "</button>"
																											+ "</a>");
																				}
											...
											onClickRow : function(row, $element) {
												...
												$
														.ajax({
															...
															success : function(
																	responseContent) { //success:function(responseContent)为回调函数   responseContent为接收响应信息
																...
																if (flag == 'true') {
																	...
																	$(
																			"#selectErrorMsg")
																			.html(
																					"<strong>警告！</strong>该任务已经在该班级发布，如果仍然选择该任务，可能会丢失之前的任务数据。请谨慎选择")
																} else if (flag == 'false') {
																	$(
																			"#selectErrorMsg")
																			.css(
	...
	function btnGroup() { // 自定义方法，添加操作按钮
		...
		let html = '<a href="####" class="btn btn-info" id="preview" data-toggle="modal" data-target="#editrole" title="修改权限">'
				+ '<span >预览</span></a>'
		return html
	};
...
<body style="background-color: #F1F3F4;">
	<div
		...
		<span
			style="line-height: 55px; font-size: 18px; margin-left: 20px; font-weight: bold;">发布任务</span>
							...
							<ul id="selectTypeEdit" class="dropdown-menu">
								<c:forEach items="${taskCategoryList }" var="taskCategory">
									<li style="font-size: 18px; padding-left: 20px"><c:if
										...
										HEAD <c:if test="${taskCategory=='turn_class' }">翻转</c:if></li>
=======
											<c:if test="${taskCategory=='turn_class' }">翻转</c:if>
									</li>
>>>>>>> edf05ff05f8d723e6c24b6ff097f1982afde3380
				...
				<div class="input-group"
					...
					<input type="text" name="knowledgePoints" class="form-control"
						placeholder="输入任务相关知识点内容" id="knowledgePoints"
				...
				<div id="preEditTime" class="input-group"
					...
					<input type="text" style="height: 30px; width: 185px"
						class="Wdate form-control" name="preTaskEndTime"
						id="taskEndTimeSelect"
				...
				<div id="editTime" class="input-group"
					...
					<input type="text" style="height: 30px; width: 185px"
						class="Wdate form-control" name="taskEndTimeClass"
						id="taskEndTimeSelect"
						onclick="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'%y-%M-%d %H:%m:%s'})"
						value="请输入时间" />
				</div>
				<div id="editTime" class="input-group"
					...
					<input type="text" style="height: 30px; width: 185px"
						class="Wdate form-control" name="taskEndTime"
						id="taskEndTimeSelect"
						onclick="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'%y-%M-%d %H:%m:%s'})"
						value="请输入时间" />
				</div>
		...
		<div class="selectTaskContent" style="margin: 30px 50px;">
			...
			<form
				...
				<div id="selectTime" class="input-group"
					...
					<input type="text" style="height: 30px; width: 185px"
						class="Wdate form-control" name="taskEndTime" id="taskEndTimeEdit"
						onclick="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'%y-%M-%d %H:%m:%s'})"
						value="请输入时间" />
				</div>
